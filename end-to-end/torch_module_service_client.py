...
service_handle = axon.client.get_ServiceStub('localhost', endpoint_prefix='module_service')
...
class FnStub(torch.autograd.Function):
	...
	def forward(ctx, x):
		...
		return service_handle.apply(ctx.id, x).join()


	@staticmethod
	def backward(ctx, g):
		return service_handle.apply_gradients(ctx.id, g).join()


async def main():

	rpcs = axon.client.get_RemoteWorker('localhost').rpcs
	remote_nn = FnStub()
	remote_optim = axon.client.get_ServiceStub('localhost', endpoint_prefix='optimizer_service')
